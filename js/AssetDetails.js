...
ACDC.AssetDetails = Ext.extend(Ext.grid.PropertyGrid, {
    ...
    initComponent: function () {
        ...
        this.propertyNames = {
            // Editable properties
            label:             "<span style='color: red;'>*</span> Label Name",  // physical label on the asset in the data center
            assetClass:        "<span style='color: red;'>*</span> Asset Class",   // translates CMDB sys class name to English text, eg., cmdb_ci_server => 'Server'
            deviceType:        "<span style='color: red;'>*</span> Device Type",

            location:          "<span style='color: red;'>*</span> Location",
            cabinet:           "<span style='color: red;'>*</span> Rack",
            elevation:         "<span style='color: red;'>*</span> Rack Elevation",
            numRUs:            "<span style='color: red;'>*</span> Number of Rack Units",
            ...
            state:             "<span style='color: red;'>*</span> State",
            powerStatus:       "<span style='color: red;'>*</span> Power Status",
        ...
        this.cabinetCombo = new ACDC.EditorComboBox({
            ...
            listeners: {
                scope:          this,
                cabinetchange: this.cabinetChange
            }
        });

        this.elevationCombo = new ACDC.EditorComboBox({
            boxMaxWidth:   boxMaxWidth,
            itemName: 'elevation',
            listeners: {
                scope:          this,
        ...
        this.numRUsCombo = new ACDC.EditorComboBox({
            boxMaxWidth:   boxMaxWidth,
            itemName: 'numrus',
            listeners: {
                scope:          this,
                numrusloaded: this.numRUsLoaded
            }
        });
        ...
        this.elevationEditor = new Ext.grid.GridEditor(this.elevationCombo);
        this.numRUsEditor = new Ext.grid.GridEditor(this.numRUsCombo);

        /*
        this.elevationEditor = new Ext.grid.GridEditor(
            // allow only numbers between 1 and 48
    ...
    showAsset: function (assetId, isBlade) {
        ...
        Ext.Ajax.request({
            ...
            mycallback:      function (json, options) {
                ...
                // define the base params for the combos location, cabinet, elevation and numRUs then load them all
                this.cabinetCombo.store.baseParams['locationName'] = this.getSource().location;
                this.cabinetCombo.store.load();

                this.elevationCombo.store.baseParams['locationName'] = this.getSource().location;
                this.elevationCombo.store.baseParams['cabinetName'] = this.getSource().cabinet;
                this.elevationCombo.store.load();

                this.numRUsCombo.store.baseParams['locationName'] = this.getSource().location;
                this.numRUsCombo.store.baseParams['cabinetName'] = this.getSource().cabinet;
                this.numRUsCombo.store.baseParams['elevation'] = this.getSource().elevation;
                this.numRUsCombo.store.load();
    ...
    locationChange: function (combo, newLocationName, oldLocationName) {
        if (newLocationName !== oldLocationName) {
            ...
            this.elevationCombo.store.baseParams['locationName'] = newLocationName;
            this.numRUsCombo.store.baseParams['locationName'] = newLocationName;

            if (this.getSource().cabinet) {
                this.elevationCombo.store.baseParams['cabinetName'] = this.getSource().cabinet;
                this.elevationCombo.store.load();

                if (this.getSource().elevation) {
                    this.numRUsCombo.store.baseParams['cabinetName'] = this.getSource().cabinet;
                    this.numRUsCombo.store.baseParams['elevation'] = this.getSource().elevation;
                    this.numRUsCombo.store.load();
                }
    ...
    cabinetChange: function (combo, newCabinetName, oldCabinetName) {
        if (newCabinetName !== oldCabinetName) {
            // add the location name to the cabinetCombo so that it's passed when the combo is activated
            if (this.getSource().location) {
                this.elevationCombo.store.baseParams['locationName'] = this.getSource().location;
                this.elevationCombo.store.baseParams['cabinetName'] = newCabinetName;
                this.elevationCombo.store.baseParams['query'] = "";
                this.elevationCombo.store.load();

                this.numRUsCombo.store.baseParams['locationName'] = this.getSource().location;

                if (this.getSource().elevation) {
                    this.numRUsCombo.store.baseParams['cabinetName'] = newCabinetName;
                    this.numRUsCombo.store.baseParams['elevation'] = this.getSource().elevation;
                    this.numRUsCombo.store.load();
                }
            }
    ...
    elevationChange: function (combo, newElevation, oldElevation) {
        if (newElevation !== oldElevation) {
            if (this.getSource().location && this.getSource().cabinet) {
                this.numRUsCombo.store.baseParams['locationName'] = this.getSource().location;
                this.numRUsCombo.store.baseParams['cabinetName'] = this.getSource().cabinet;
                this.numRUsCombo.store.baseParams['elevation'] = newElevation;
                this.numRUsCombo.store.baseParams['query'] = "";
                this.numRUsCombo.store.load();
            }
        }
    },

    numRUsLoaded: function(combo, records, options) {
        // this does not work. Should put in the first value of the return records, but does not
        //combo.setValue(records[0].id);
        //this.propStore.getSource().numRUs = records[0].name;
    },
    ...
    renderEditable: function (value, metadata, record, rowIndex, colIndex, store) {
        var items = this.grid.locationCombo.store.data.items;
        if (value == 0) {
    ...
    checkSerialNumber: function(record, oldValue, newValue) {
        Ext.Ajax.request({
            ...
            scope:           this,
            mycallback:      function (json, options) {
                /** @namespace json.found */
                /** @namespace json.label */
    ...
    installAsset: function (locationName) {
        var src = {},
            searchForm;
                        ...
                        handler:  function () {
                            ...
                            if (Ext.getCmp('asset-name').getValue() !== '') {
                                ...
                                */
                                ...
                                this.locationChange(this.cabinetCombo, locationName, "");
                            }
    ...
    ajaxSave: function(params, changes, modifiedItems) {
        ...
        Ext.Ajax.request({
            ...
            scope:           this,
            mycallback:      function (json, options) {
                /** @namespace json.newlyFoundInCmdb */
