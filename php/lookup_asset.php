...
try {
    ...
    for ($i = 0; $i < count($cis); $i++) {
        ...
        $items[] = array(
            ...
            'cabinet'           => $cmdbCI->getRack() ? $cmdbCI->getRack() : '',
            'elevation'         => $cmdbCI->getRackPosition() ? $cmdbCI->getRackPosition() : 0,
            'numRUs'            => $cmdbCI->getNumberOfRackUnits() ? $cmdbCI->getNumberOfRackUnits() : 2,

            'deviceType'        => $cmdbCI->getSysClassName() == 'cmdb_ci_server' ? $cmdbCI->getHostType() : $cmdbCI->getDeviceType(),
            'manufacturer'      => $cmdbCI->getManufacturer() ? $cmdbCI->getManufacturer() : '',
            'model'             => $cmdbCI->getModelNumber() ? $cmdbCI->getModelNumber() : '',
