...
try {
    ...
    switch ($params['action']) {
        ...
        case 'update':
             ...
             *      ACDC State              CMDB Status
             *      In Transit                          Staging
             *      Repurposed                          No Change
             *      Relocated On-Site                   No Change
             *      Relocated Off-Site                  No Change
             *      Inventory (was Decommed On-Tile)    Inventory
             *      Decommed Off-Tile (deleted)         -
             *      Decommed Off-Site                   Decommissioning
             *      Awaiting Disposal                   Decommissioning
             *      Disposed                            Disposed
             */
            if ($params['isBlade']) {
            ...
            if ($location->getName() != "" && $location->getName() != null && $location->getName() != $params['location']) {
                ...
                // In Transit, Inventory, Decommed Off-Site, Awaiting Disposal or Disposed
                $action = $actionTable->createEntry($asset->getId(), $asset->getSysId(), $actor->getId(), $params['state']);

                // if asset is in transit, decommed off-tile, awaiting disposal or disposed,
                // clear asset cabinet id and elevation
                // clear CMDB CI location, rack and elevation
                if ($params['state'] != 'Inventory') {
            ...
            if ($cmdbCI->getSysId()) {
                ...
                else if ($action->getAction() == ActionTable::INVENTORY) {
                    // CMDB CI status = Inventory
                    $cmdbAsset->setInstallStatusId($cmdbStatusHash['Inventory']);
                    $asset->setInstallStatus('Inventory');
                }
                else if ($action->getAction() == ActionTable::DECOMMED_OFFSITE) {
        ...
        'install':
            ...
            switch ($params['source']) {
                case 'cmdb':
                    ...
                    // if the current CMDB asset has the asset tag as the name, update the name with the asset name or label name
                    // asset tag is 6 numbers
                    if (preg_match("/\d\d\d\d\d\d/", $cmdbAsset->getName())) {
                        if ($params['name']) {
                            $cmdbAsset->setName($params['name']);
                        } else {
                            $cmdbAsset->setName($params['label']);
                        }
                    }
                    // if SAN switch, there is no device type, so check before we assign
                    if ($params['deviceType'] && array_key_exists($params['deviceType'], $cmdbDeviceTypeHash)) {
            ...
            $asset = $assetTable->getByLabelAndLocation($params['label'], $params['location']);
            if ($asset->getId()) {
                ...
                $location = $locationTable->getByName($params['location']);
                if ($location->getId()) {
                    $cabinet = $cabinetTable->getByNameAndLocationId($params['cabinet'], $location->getId());
                } else {
                    $cabinet = new Cabinet();
                }
                $asset    = new Asset();
            ...
            // if this is fullfilling a reservation, then remove the reservation now
            $resTable = new RUReservationTable();
            $res = $resTable->getByCabinetIdAndElevation($cabinet->getId(), $asset->getElevation());
            if ($res->getId()) {
                $resTable->delete($res);
            }
